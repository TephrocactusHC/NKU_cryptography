...
#define MAX_BYTES 64
  ...
  if (nbytes > MAX_BYTES) {
    fprintf(stderr,
            "ERROR: bin_to_string supports up to %d bytes.  %d bytes given.\n",
            MAX_BYTES, nbytes);
  }
  sprintf(output, "0x");
  output_ptr += 2;
...
void xor
    (const unsigned char* block_a, const unsigned char* block_b,
 ...
 * the left separately.
 */
void des_key_shift(unsigned char key[7], unsigned char output[7],
...
void des_substitution_box(const unsigned char input[6],
...
void des_feistel(const unsigned char input[4], const unsigned char subkey[6],
                 unsigned char output[4]) {
...
void des_encrypt(unsigned char block[8], unsigned char key[8],
