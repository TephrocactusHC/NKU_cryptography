...
int cmp(const void *a, const void *b)//单词 按次数从大到小排列
{
...
single_table wordAttack(single_table tableAttack, p_word word[]) {
	...
	file.close();
	qsort(&it[0], it.size(), sizeof(word_count), cmp);
	//sortWord(it);
...
//void sortWord(vector<word_count> it) {
//	int i,j,k;
//	for (i = 0; i < it.size()-1; i++) {
//		k = i;
//		for (j = i+1; j < it.size(); j++) 
//			if (it[j].count > it[k].count)
//				k = j;
//		swapWordP(it, i, k);
//			/*else if (it[i].count == it[j].count&&it[i].word.compare(it[j].word) > 0) 
//					compareWordP(it, i, j);*/
//
//		
//	}
